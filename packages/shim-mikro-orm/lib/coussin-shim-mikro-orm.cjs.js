...
class MikroOrmShim extends Shim {
    ...
    specialTypes() {
        const result = this._entities.map((entity) => ({
            ...
            toJSON: (data) => {
                const cacheEntity = {
                    target: core.wrap(data)?.toJSON() ?? data,
                    populate: core.wrap(data, true)?.__serializationContext.populate,
                };
                return cacheEntity;
            },
            toObject: (data) => {
                ...
                for (const key in target) {
                    ...
                    core.wrap(element)?.populated?.(populateFields.includes(key));
                }
                core.wrap(entityInstance)?.populated?.(false);
    ...
    collect(target, idMapEntity = new Map(), path = "$") {
        ...
        if ((target instanceof core.Collection && target.isInitialized()) ||
...
exports.MikroOrmShim = MikroOrmShim;
